...
\begin{itemize}[leftmargin=*]
    ...
    Data pocházejí z~portálu Open Data hlavního města Prahy a jsou vedena pod názvem \emph{Dopravní přestupky MHMP}. Pro trénování modelu byla zvolena data z~roku 2023, zatímco pro validaci a testování byly využity sady z~let 2022 a 2024. dodatečně jsme pak použili data z roků 2021 a 2020 pro další testování.
\newpage
    \noindent \textbf{Odkazy na zdroje:}
    \begin{itemize}
        \item Rok 2024: \href{https://opendata.praha.eu/datasets/https%3A%2F%2Fapi.opendata.praha.eu%2Flod%2Fcatalog%2F8887b335-4108-447d-9b86-0729fd06913e}{Dopravní přestupky MHMP 2024}
        \item Rok 2023: \href{https://opendata.praha.eu/datasets/https%3A%2F%2Fapi.opendata.praha.eu%2Flod%2Fcatalog%2F1aba6dfc-0662-4dca-9326-07698e9af0fb}{Dopravní přestupky MHMP 2023}
        \item Rok 2022: \href{https://opendata.praha.eu/datasets/https%3A%2F%2Fapi.opendata.praha.eu%2Flod%2Fcatalog%2Fa6893355-51fc-4904-89cd-716a4588abed}{Dopravní přestupky MHMP 2022}
        \item Rok 2022: \href{https://opendata.praha.eu/datasets/https%3A%2F%2Fapi.opendata.praha.eu%2Flod%2Fcatalog%2F7e4143f3-567e-425e-86bf-0cb92b08f1e3}{Dopravní přestupky MHMP 2021}
        \item Rok 2022: \href{https://opendata.praha.eu/datasets/https%3A%2F%2Fapi.opendata.praha.eu%2Flod%2Fcatalog%2Fd7468a8b-0ef0-425d-bc21-d4193055b195}{Dopravní přestupky MHMP 2020}
    \end{itemize}
...
\section{Experimenty a výsledky}
    ...
    Všechny modely měly přesnost 99 \,\% při určování záznamů od MPP ale při určování PČR byla přesnost u některých modelů výrazně nižší (nejhorší 61 \,\% měl model s jednou skrytou vrstvou na datech z roku 2022). Graf Přesnosti na třídě PČR vidíme na obrázku ~\ref{fig:precision_graph} níže.
    
    \begin{figure}[h!]
        \centering
        \includegraphics[width=1\linewidth]{precision_graph.png}
        \caption{Graf přesnosti na třídě PČR}
        \label{fig:precision_graph}
    \end{figure}
...
\begin{table}[h!]
    ...
    \begin{tabular}{l|c c c c}
        ...
        \hline
        2024 & 0.95 & 0.96 & 0.89 & \textbf{0.99} \\
        \hline
        2023 - trénink & 0.99 & 0.99 & 0.99 & \textbf{0.99} \\
        \hline
        2022 & 0.61 & 0.82 & 0.83 & \textbf{0.99} \\
        \hline
        2021 & 0.73 & 0.88 & 0.84 & \textbf{0.94} \\
        \hline
        2020 & 0.77 & 0.89 & 0.86 & \textbf{0.98} \\
    \end{tabular}
\end{table}
...
\begin{itemize}[leftmargin=*]
    ...
    \begin{itemize}
        \item \textbf{Precision (0,97):} Tato metrika říká, že v případech kde model označil záznam jako PČR, tak se v 97\% trefil. Pokud by se model snažil násilně vyhledávat PČR, mohlo by vzniknout více případů FP (false positive), k tomu však nedošlo.
        \item \textbf{Recall (0,95):} Recall 95\% pro třídu PČR znamená, že z celkového počtu záznamů PČR v datasetu jich model objevil 95\%. Pokud by model raději označoval MPP kvůli jejich velkému zastoupení, bylo by číslo nižší.
        \item \textbf{F1-Score (0,96):} Parametr F1-Score je harmonickým průměrem parametrů Precision a Recall. Nízká hodnota by mohla naznačovat náchylnost vůči datové nevyváženosti. Tento výsledek ale ukazuje, že náš model je proti ní velmi naopak velmi odolný.
    \end{itemize}
...
\newpage
    ...
    Nevýhodou neuronové sítě je že nevidíme přesně, proč se rozhodla tak jak se rozhodla. Není proto možné z výsledků s jistotou vyčíst nějaký patern pro rozdělení. Kdybychom chtěli větší transparentnost, mohli bychom příště zkusit \textbf{Rozhodovací stromy (Decision Trees)}. Ty by nám formou diagramu přesně ukázaly pravidla (např. „pokud chybí čas $\to$ je to PČR“), což by bylo pro vysvětlování chování policie srozumitelnější.
...
\section*{Přílohy}
\begin{itemize}[leftmargin=*]
    \item \textbf{Výstup profilování dat} (ydata-profiling): \texttt{vystup\_report.html}
    \item \textbf{Vybrané části kódu}: \texttt{data\_process.py}, \texttt{train.py}, \texttt{predict.py}
\end{itemize}
